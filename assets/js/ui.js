...
let flowLastRemoteDataHealth = null;
...
function renderDataHealth(local,remote){
    const title=document.getElementById('data-health-title'),badge=document.getElementById('data-health-badge'),
      ...
      cleanBtn=document.getElementById('historical-clean-btn');
    if(!title||!badge||!summary||!issues||!last)return;

    flowLastRemoteDataHealth=remote||null;
    const cloudOk=Boolean(remote&&remote.status==='success');
    const syncOk=Number(local.pending||0)===0;
    const ok=local.ok && cloudOk;
    ...
    if(remote&&remote.status!=='success') all.push('Cloudový stav sa nepodarilo overiť');
    ...
    if(!syncOk) all.push('Čistenie je zablokované, kým sa nedokončí synchronizácia');
    ...
    else if(cloudOk)last.textContent='Zatiaľ nevytvorená'; else last.textContent=remote?.message||'Cloud sa nepodarilo overiť';

    if(auto){
      if(remote?.autoBackupEnabled===true) auto.textContent='Zapnutá · posledný deň mesiaca';
      else if(remote?.autoBackupEnabled===false) auto.textContent='Vypnutá';
      else auto.textContent='Neoverené';
    }

    // Never allow destructive cleanup while local mutations are waiting or
    // while the cloud cannot be verified.
    if(cleanBtn){
      const blocked=!cloudOk || !syncOk;
      cleanBtn.disabled=blocked;
      cleanBtn.classList.toggle('opacity-50',blocked);
      cleanBtn.classList.toggle('cursor-not-allowed',blocked);
      cleanBtn.title=!cloudOk?'Najprv musí byť dostupný a overený cloud':!syncOk?'Najprv dokonči synchronizáciu':'Bezpečné čistenie s povinnou zálohou';
    }
...
async function cleanHistoricalDuplicates(btn){
    if(btn){btn.disabled=true;btn.classList.add('opacity-60');}
    try{
      // 1) Local queue must be empty. Try to finish it once automatically.
      let pending=Array.isArray(syncQueue)?syncQueue.length:0;
      if(pending>0){
        showToast({type:'info',title:'Najprv dokončujem synchronizáciu',text:`Čaká ${pending} zmien. Čistenie sa nespustí, kým nebudú v cloude.`});
        ...
        pending=Array.isArray(syncQueue)?syncQueue.length:0;
        if(pending>0) throw new Error(`Stále čaká ${pending} zmien na synchronizáciu. Historické dáta som nemenil.`);
      }

      // 2) Cloud health must succeed independently of trigger permissions.
      const healthResponse=await fetch(buildSyncGetUrl('data_health'),{cache:'no-store'});
      const healthText=await healthResponse.text(); let health;
      try{health=JSON.parse(healthText);}catch(_){throw new Error(`Neplatná odpoveď servera (${healthResponse.status})`);}
      if(!healthResponse.ok||health?.status!=='success') throw new Error(health?.message||'Cloudový stav sa nepodarilo bezpečne overiť.');

      // 3) Audit only after sync + cloud preflight.
      ...
      const ok=confirm(`Flow našiel ${audit.removeCount} duplicitných historických riadkov v ${audit.groups} skupinách.\n\nPred čistením server ešte raz overí bezpečnostné podmienky a vytvorí kompletnú cloudovú zálohu. Bežné transakcie ID-* sa nebudú mazať.\n\nPokračovať?`);
      if(!ok) return;

      showToast({type:'info',title:'Bezpečné čistenie',text:'Vytváram povinnú zálohu a až potom odstránim potvrdené HIST-* duplicity.'});
      const r=await fetch(GOOGLE_URL,{
        method:'POST',
        headers:{'Content-Type':'text/plain;charset=utf-8'},
        body:JSON.stringify({
          action:'cleanHistoricalDuplicates',
          token:getSyncToken(),
          userId:typeof FLOW_USER_ID!=='undefined'?FLOW_USER_ID:'default',
          syncQueueCount:Array.isArray(syncQueue)?syncQueue.length:0,
          expectedDuplicateRows:Number(audit.removeCount||0)
        })
      });
