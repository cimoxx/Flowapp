...
const FLOW_BACKEND_VERSION = '2.50.0';
// Google OAuth Web Client ID. Create a Web application credential in Google Cloud
// and paste it here, then run setupFlowAuth() once from the Apps Script editor.
const FLOW_GOOGLE_CLIENT_ID = 'PASTE_GOOGLE_WEB_CLIENT_ID_HERE.apps.googleusercontent.com';

// Accounts allowed to bootstrap the private FlowUsers sheet.
// Replace these examples before running setupFlowAuth().
const FLOW_BOOTSTRAP_EMAILS = [
  'PASTE_ALLOWED_GOOGLE_EMAIL_1_HERE'
];

const FLOW_SESSION_DAYS = 7;
...
function ensureAuthSheets_(ss) {
  const users=ensureSheet_(ss,'FlowUsers',['Email','GoogleSub','Name','Role','Active','LastLogin']);
  const sessions=ensureSheet_(ss,'FlowSessions',['TokenHash','GoogleSub','Email','Name','Picture','ExpiresAt','Active','CreatedAt']);
  return {users:users,sessions:sessions};
}

function setupFlowAuth() {
  const ss=SpreadsheetApp.getActiveSpreadsheet();
  const sheets=ensureAuthSheets_(ss);
  const existing=sheets.users.getLastRow()>1
    ? sheets.users.getRange(2,1,sheets.users.getLastRow()-1,6).getValues()
    : [];
  const existingEmails=new Set(existing.map(function(r){return String(r[0]||'').trim().toLowerCase();}));

  FLOW_BOOTSTRAP_EMAILS.forEach(function(email){
    const value=String(email||'').trim().toLowerCase();
    if(!value || value.indexOf('paste_')===0 || existingEmails.has(value)) return;
    sheets.users.appendRow([value,'','',existing.length?'member':'owner',true,'']);
    existingEmails.add(value);
  });

  return {
    configured:FLOW_GOOGLE_CLIENT_ID.indexOf('PASTE_')!==0,
    users:Math.max(0,sheets.users.getLastRow()-1)
  };
}
...
function verifyGoogleIdToken_(credential) {
  if(!credential) throw new Error('Chýba Google credential');
  if(!FLOW_GOOGLE_CLIENT_ID || FLOW_GOOGLE_CLIENT_ID.indexOf('PASTE_')===0) throw new Error('Google login nie je nakonfigurovaný');

  const url='https://oauth2.googleapis.com/tokeninfo?id_token='+encodeURIComponent(String(credential));
  const response=UrlFetchApp.fetch(url,{muteHttpExceptions:true});
  if(response.getResponseCode()!==200) throw new Error('Google účet sa nepodarilo overiť');
  const payload=JSON.parse(response.getContentText()||'{}');

  if(String(payload.aud||'')!==FLOW_GOOGLE_CLIENT_ID) throw new Error('Neplatný Google Client ID');
  if(!['accounts.google.com','https://accounts.google.com'].includes(String(payload.iss||''))) throw new Error('Neplatný vydavateľ Google tokenu');
  if(Number(payload.exp||0)*1000<=Date.now()) throw new Error('Google prihlásenie vypršalo');
  if(String(payload.email_verified||'')!=='true' && payload.email_verified!==true) throw new Error('Google email nie je overený');

  return {
    sub:String(payload.sub||''),
    email:String(payload.email||'').trim().toLowerCase(),
    name:String(payload.name||payload.given_name||payload.email||'Google účet'),
    picture:String(payload.picture||'')
  };
}

function getAuthorizedUser_(ss,googleUser) {
  const sheets=ensureAuthSheets_(ss);
  if(sheets.users.getLastRow()<2) return null;
  const rows=sheets.users.getRange(2,1,sheets.users.getLastRow()-1,6).getValues();

  let matchIndex=-1;
  for(let i=0;i<rows.length;i++){
    const email=String(rows[i][0]||'').trim().toLowerCase();
    const sub=String(rows[i][1]||'').trim();
    const active=rows[i][4]===true || String(rows[i][4]).toLowerCase()==='true';
    if(!active) continue;
    if(sub && sub===googleUser.sub){matchIndex=i;break;}
    if(!sub && email && email===googleUser.email){matchIndex=i;break;}
  }
  if(matchIndex<0) return null;

  const row=rows[matchIndex];
  // Link the stable Google `sub` on first successful login.
  if(!String(row[1]||'').trim()) sheets.users.getRange(matchIndex+2,2).setValue(googleUser.sub);
  sheets.users.getRange(matchIndex+2,3).setValue(googleUser.name);
  sheets.users.getRange(matchIndex+2,6).setValue(nowIso_());

  return {
    sub:googleUser.sub,email:googleUser.email,name:googleUser.name,picture:googleUser.picture,
    role:String(row[3]||'member')
  };
}

function createFlowSession_(ss,user) {
  const sheets=ensureAuthSheets_(ss);
  const raw=Utilities.getUuid()+'_'+Utilities.getUuid()+'_'+Date.now();
  const hash=hashSessionToken_(raw);
  const expires=new Date(Date.now()+FLOW_SESSION_DAYS*86400000).toISOString();
  sheets.sessions.appendRow([hash,user.sub,user.email,user.name,user.picture,expires,true,nowIso_()]);
  return {token:raw,expiresAt:expires};
}

function getSessionUser_(ss,token) {
  if(!token) return null;
  const cache=CacheService.getScriptCache();
  const hash=hashSessionToken_(token);
  const cached=cache.get('flow_session_'+hash);
  if(cached){try{return JSON.parse(cached);}catch(_){}}

  const sheets=ensureAuthSheets_(ss);
  if(sheets.sessions.getLastRow()<2) return null;
  const rows=sheets.sessions.getRange(2,1,sheets.sessions.getLastRow()-1,8).getValues();
  for(let i=rows.length-1;i>=0;i--){
    if(String(rows[i][0]||'')!==hash) continue;
    const active=rows[i][6]===true || String(rows[i][6]).toLowerCase()==='true';
    const expires=new Date(rows[i][5]).getTime();
    if(!active || !Number.isFinite(expires) || expires<=Date.now()) return null;
    const user={sub:String(rows[i][1]||''),email:String(rows[i][2]||''),name:String(rows[i][3]||''),picture:String(rows[i][4]||'')};
    cache.put('flow_session_'+hash,JSON.stringify(user),600);
    return user;
  }
  return null;
}

function getRequestToken_(eOrItem) {
  return eOrItem && eOrItem.parameter ? String(eOrItem.parameter.token||'') : String(eOrItem&&eOrItem.token||'');
}

function validToken_(eOrItem) {
  const ss=SpreadsheetApp.getActiveSpreadsheet();
  return Boolean(getSessionUser_(ss,getRequestToken_(eOrItem)));
}

function processGoogleLogin_(ss,credential) {
  const googleUser=verifyGoogleIdToken_(credential);
  const user=getAuthorizedUser_(ss,googleUser);
  if(!user) return {status:'error',message:'Tento Google účet nemá povolený prístup do Flow.'};
  const session=createFlowSession_(ss,user);
  return {status:'success',sessionToken:session.token,expiresAt:session.expiresAt,user:user,backendVersion:FLOW_BACKEND_VERSION};
}

function logoutFlowSession_(ss,token) {
  if(!token) return {status:'success'};
  const hash=hashSessionToken_(token);
  const sheets=ensureAuthSheets_(ss);
  if(sheets.sessions.getLastRow()>=2){
    const rows=sheets.sessions.getRange(2,1,sheets.sessions.getLastRow()-1,8).getValues();
    for(let i=rows.length-1;i>=0;i--){
      if(String(rows[i][0]||'')===hash){sheets.sessions.getRange(i+2,7).setValue(false);break;}
    }
  }
  CacheService.getScriptCache().remove('flow_session_'+hash);
...
function doGet(e) {
  ...
  // Public configuration contains no financial data.
  if (get === 'auth_config') {
    return json_({
      status:'success',
      configured:Boolean(FLOW_GOOGLE_CLIENT_ID && FLOW_GOOGLE_CLIENT_ID.indexOf('PASTE_')!==0),
      clientId:FLOW_GOOGLE_CLIENT_ID.indexOf('PASTE_')===0 ? '' : FLOW_GOOGLE_CLIENT_ID,
      backendVersion:FLOW_BACKEND_VERSION
    });
  }

  if (!validToken_(e)) return json_({ status:'error', message:'Prihlásenie je potrebné' });
  const ss = SpreadsheetApp.getActiveSpreadsheet();

  try {
    if (get === 'auth_status') {
      const user=getSessionUser_(ss,getRequestToken_(e));
      return json_({status:'success',authenticated:Boolean(user),user:user,backendVersion:FLOW_BACKEND_VERSION});
    }
...
function doPost(e) {
  ...
  try {
    ...
    // Google login is the only public POST action.
    if (item.action === 'googleLogin') return json_(processGoogleLogin_(ss,item.credential));

    if (!validToken_(item)) return json_({ status:'error', message:'Prihlásenie je potrebné' });
    if (item.action === 'logout') return json_(logoutFlowSession_(ss,item.token));
