...
const FLOW_BACKEND_VERSION = '2.49.1';
...
function createFullBackup_(ss,userId,reason) {
  ...
  // Use the Spreadsheet service itself instead of DriveApp.
  // This avoids the extra Drive authorization that caused backup failures
  // on existing web-app deployments.
  const copy=ss.copy(name);

  const log=ensureBackupLog_(ss);
  log.appendRow([nowIso_(),copy.getId(),name,String(userId||''),String(reason||'manual'),'ok']);

  // Keep a rolling history in the log. We deliberately do not trash older
  // copies automatically here; deleting Drive files would require DriveApp.
  ...
  if(okRows.length>30){
    okRows.slice(0,okRows.length-30).forEach(x=>log.getRange(x.row,6).setValue('archived'));
  }

  return {status:'success',createdAt:nowIso_(),fileId:copy.getId(),name:name,backendVersion:FLOW_BACKEND_VERSION};
}
