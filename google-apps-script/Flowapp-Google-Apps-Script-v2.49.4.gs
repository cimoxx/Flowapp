...
const FLOW_BACKEND_VERSION = '2.49.4';
...
function cleanHistoricalDuplicates_(ss,userId) {
  const audit=auditHistoricalDuplicates_(ss);
  if(audit.status!=='success' || !audit.removeCount) return {...audit,cleaned:0,backup:null};
  ...
  if(!backup || backup.status!=='success') return {status:'error',message:'Bezpečnostná záloha zlyhala. Sheet1 zostal nezmenený.',backup:backup||null};

  const fullAudit=auditHistoricalDuplicates_(ss);
  const rows=[];
  (fullAudit.duplicates||[]).forEach(g=>(g.removeRows||[]).forEach(r=>rows.push(r)));
  // audit response is capped for UI, so rebuild all groups when needed.
  const sheet=ss.getSheetByName('Sheet1');
  const values=sheet.getDataRange().getValues();
  const groups=new Map();
  values.slice(1).forEach((row,i)=>{
    if(!row[0]) return;
    const tx=transactionFromRow_(row); if(tx.deleted) return;
    const sig=historicalDuplicateSignature_(tx); if(!sig) return;
    const list=groups.get(sig)||[]; list.push({rowIndex:i+2,tx:tx}); groups.set(sig,list);
  });
  const allRows=[];
  groups.forEach(list=>{
    if(list.length<2) return;
    const ranked=list.slice().sort((a,b)=>historicalSurvivorScore_(b.tx)-historicalSurvivorScore_(a.tx) || a.rowIndex-b.rowIndex);
    ranked.slice(1).forEach(x=>allRows.push(x.rowIndex));
  });
  [...new Set(allRows)].sort((a,b)=>b-a).forEach(rowIndex=>sheet.deleteRow(rowIndex));
  return {status:'success',cleaned:[...new Set(allRows)].length,groups:audit.groups,backup:backup,remaining:auditHistoricalDuplicates_(ss).removeCount};
}
...
function doPost(e) {
  ...
  try {
    ...
    if (item.action === 'cleanHistoricalDuplicates') return json_(cleanHistoricalDuplicates_(ss,item.userId));
