...
const FLOW_BACKEND_VERSION = '2.49.0';
...
function getBackupState_(ss) {
  ...
  const last=rows[rows.length-1]; return {count:rows.length,lastBackupAt:toIso_(last[0],''),lastBackupName:String(last[2]||'')};
}
function createFullBackup_(ss,userId,reason) {
  ...
  const log=ensureBackupLog_(ss); log.appendRow([nowIso_(),copy.getId(),name,String(userId||''),String(reason||'manual'),'ok']);
  const rows=log.getRange(2,1,Math.max(0,log.getLastRow()-1),6).getValues();
  const okRows=rows.map((r,idx)=>({row:idx+2,data:r})).filter(x=>String(x.data[5]||'')==='ok'&&x.data[1]);
  if(okRows.length>30)okRows.slice(0,okRows.length-30).forEach(x=>{try{DriveApp.getFileById(String(x.data[1])).setTrashed(true);}catch(_){} log.getRange(x.row,6).setValue('expired');});
  return {status:'success',createdAt:nowIso_(),fileId:copy.getId(),name:name};
}
function getDataHealth_(ss) {
  const tx=ss.getSheetByName('Sheet1'),cat=readCategoriesState_(ss),planning=getPlanningData_(ss,''),backup=getBackupState_(ss);
  return {status:'success',backendVersion:FLOW_BACKEND_VERSION,transactions:tx?Math.max(0,tx.getLastRow()-1):0,
    categories:Array.isArray(cat.categories)?cat.categories.length:0,recurring:Array.isArray(planning.recurring)?planning.recurring.length:0,
    events:Array.isArray(planning.events)?planning.events.length:0,lastBackupAt:backup.lastBackupAt,lastBackupName:backup.lastBackupName,backupCount:backup.count};
}
