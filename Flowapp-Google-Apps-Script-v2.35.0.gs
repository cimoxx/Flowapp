...
const FLOW_BACKEND_VERSION = '2.35.0';
...
const PLANNING_HEADERS_ = {
  ...
  archive: ['id','targetMonth','category','forecastAmount','budgetAmount','actualAmount','modelVersion','generatedAt','dataMonths','confidence','method','inputsJson','errorAmount','absoluteError','errorPct','evaluatedAt'],
...
function getPlanningData_(ss) {
  ...
  if (archive.getLastRow() >= 2) {
    out.archive = archive.getRange(2,1,archive.getLastRow()-1,PLANNING_HEADERS_.archive.length).getValues().filter(r=>r[0]).map(r=>rowToObject_(PLANNING_HEADERS_.archive,r));
  }
...
function archiveForecasts_(ss, rows) {
  ...
  if (sheet.getLastRow() >= 2) {
    sheet.getRange(2,1,sheet.getLastRow()-1,PLANNING_HEADERS_.archive.length).getValues().forEach(r=>{ if(r[0]) existing.add(String(r[0])); });
  }
  let saved=0;
  rows.filter(r=>r && r.id).forEach(r=>{
    const idx=findById_(sheet,r.id);
    const row=objectToRow_('archive',r);
    if(idx>-1) sheet.getRange(idx,1,1,row.length).setValues([row]);
    else sheet.appendRow(row);
    saved++;
  });
...
function doGet(e) {
  ...
  try {
    ...
    if (get === 'planning') {
      return json_(getPlanningData_(ss));
    }
